...
public class XWalkEmbeddedAPISample extends ListActivity {
    ...
    private String[][] TITLES = {{"Usecase EmbeddingAPI"}, {"XWalkView", "XWalkview-Extended", "XwalkUICilent & XwalkResourceClient", "Misc", "Setting"}};

    private String[] CATEGORIES = {"XWalkView.Basic", "XWalkView.Extended", "XWalkView.UIClient.ResourceClient", "XWalkView.Misc", "XWalkView.Setting"};
        ...
        public SampleAdapter(int level, int order){
            ...
            if(level == 0){
                for(int i = 0; i < TITLES[1].length; i++) {
                    Intent target = new Intent();
                    target.setClassName("org.xwalk.embedded.api.sample",
                            "org.xwalk.embedded.api.sample.XWalkEmbeddedAPISample");
