...
do
    if [ -d $BUILD_DEST/opt/$name/$folderName/$buildfolder ];then
        if [ $buildfolder == "dynamic" ];then
            cd $xpkpacktooldir
            python make_apk.py --package=org.xwalk.$buildfolder"2D" --name=$buildfolder"2D" --app-root=$BUILD_DEST/opt/$name/$folderName/$buildfolder --app-local-path=index.html --mode=$mode
            if [ $? -ne 0 ];then
...
# zip for resource
mv $xpkpacktooldir/*.apk $BUILD_DEST/opt/$name/$folderName
cd $BUILD_DEST
...
# zip_for_xpk
#mv *.apk $BUILD_DEST/opt/$name/$folderName
