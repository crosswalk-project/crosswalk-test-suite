...
PATCH_DIR="/home/orange/01_qiuzhong/04-work/patch/aio/32"
...
init_ww(){
    ...
    if [ $(date +%w) -eq 4 ];then
        mkdir -p $1/android/{$BRANCH_TYPE/$VERSION_NO/{testsuites-embedded/{x86,arm},testsuites-shared/{x86,arm},cordova4.x-embedded/{x86,arm},cordova4.x-shared/{x86,arm}},beta}
    else
        mkdir -p $1/android/{$BRANCH_TYPE/$VERSION_NO/{testsuites-embedded/{x86,arm},testsuites-shared/{x86,arm},cordova4.x-embedded/{x86,arm},cordova4.x-shared/{x86,arm}},beta}
    fi
    ...
    CORDOVA4_EMBEDDED_DIR=$1/android/$BRANCH_TYPE/$VERSION_NO/cordova4.x-embedded/
    CORDOVA4_SHARED_DIR=$1/android/$BRANCH_TYPE/$VERSION_NO/cordova4.x-shared/
    ...
    [ ! -f $ANDROID_IN_PROCESS_FLAG64 ] && touch $ANDROID_IN_PROCESS_FLAG64
...
prepare_tools(){
    ...
    if [ $# -ge 2 ];then
        ...
        if [[ $2 == "cordova4.x" ]];then
                rm -rf cordova
                rm -rf cordova_plugins
                cd ${PKG_TOOLS}/cordova_plugins_4.0/cordova-plugin-crosswalk-webview
                git pull
                cd -
                ...
                # Checkout to crosswalk-18 temporarily.
                cd $CTS_DIR/tools/cordova_plugins/cordova-plugin-crosswalk-webview;git reset --hard HEAD;git checkout crosswalk-18;git pull
                # End
                ...
                if [[ $3 == "shared" ]]; then
                    if [  -f "${PKG_TOOLS}/crosswalk-shared-${VERSION_NO}.aar" ]; then
                        mvn install:install-file -DgroupId=org.xwalk -DartifactId=xwalk_shared_library -Dversion=${VERSION_NO} -Dpackaging=aar  -Dfile=${PKG_TOOLS}/crosswalk-shared-${VERSION_NO}.aar -DgeneratePom=true
                    fi
                else
...
save_Package(){
    ...
    mkdir -p /mnt/otcqa/2016/$wdir/{$BRANCH_TYPE/"ww"$wweek"."$wtoday,stable,webtestingservice}
    if [ $wtoday -eq 5 ];then
        fulltest_dir=/mnt/otcqa/2016/$wdir/$BRANCH_TYPE/"ww"$wweek"."$wtoday/FullTest
        mkdir -p $fulltest_dir
        cp -r $EMBEDDED_TESTS_DIR $fulltest_dir/
        cp -r $SHARED_TESTS_DIR $fulltest_dir/
        cp -r $EMBEDDED_TESTS_DIR/../cordova* $fulltest_dir/
...
# Start 2015-11-25: No needs to pack cordova 3.6 anymore.
...
# End 2015-11-25: No needs to pack cordova 3.6 anymore.

prepare_tools x86 cordova4.x embedded

# pack_Cordova x86 embedded 4.x &
# pack_Cordova_SampleApp x86 embedded 4.x &
# pack_Aio cordova4.x x86 embedded &
# wait

pack_Cordova arm embedded 4.x &
pack_Cordova_SampleApp arm embedded 4.x &
pack_Aio cordova4.x arm embedded &
wait

#START# add cordova4.x share mode [yunfei] 2015.08.26
prepare_tools x86 cordova4.x shared

# pack_Cordova x86 shared 4.x &
# pack_Cordova_SampleApp x86 shared 4.x &
# pack_Aio cordova4.x x86 shared &
# wait

pack_Cordova arm shared 4.x &
pack_Cordova_SampleApp arm shared 4.x &
pack_Aio cordova4.x arm shared &
wait
...
/home/orange/01_qiuzhong/04-work/scripts/circ_eh_master.sh ${VERSION_NO}
