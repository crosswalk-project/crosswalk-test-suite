...
class all_pairs2:
    ...
    def __init__(
            self, options, filter_func=lambda x: True, previously_tested=[[]], n=2):
        """
        ...
        for arr in previously_tested:
            ...
            for i, val in enumerate(arr):
                idxs = [
                    item(
                        node.id,
                        0) for node in self.__working_arr[i] if node.value == val]
                if len(idxs) != 1:
                    raise Exception(
    ...
    def next(self):
        ...
        while -1 < i < len(self.__working_arr):
            ...
            if self.__filter_func(
                    self.get_values_array(chosen_values_arr[:i + 1])):
                assert(direction > -1)
                direction = 1
            else:
                direction = 0
    ...
    def resort_working_array(self, chosen_values_arr, num):
        for item in self.__working_arr[num]:
            ...
            for i in range(0, self.__n):
                ...
                # appended to array
                new_combs.append(set([pairs_storage.key(z) for z in xuniqueCombinations(
                    chosen_values_arr + [item], i + 1)]) - self.__pairs.get_combs()[i])
            # weighting the node
