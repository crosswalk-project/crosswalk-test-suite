    ...
    try:
        ...
        if (pakeNo == "all"):  # all is not support now,please use default
            for i in range(1, (Manifest_Row + 1)):
                cmd_line = " ./tcs/Crosswalk-Manifest-Check" + \
                    str(i) + " -o ./opt/wrt-manifest-tizen-tests/Crosswalk-Manifest-Check" + \
                    str(i) + "." + pakeType + " key.pem "
                if (pakeType == "xpk"):
                    ...
                    os.chdir(
                        ...
                        str(i))
                    os.system(
                        "zip -rq ../../opt/wrt-manifest-tizen-tests/Crosswalk-Manifest-Check" +
                        str(i) +
                        "." +
                        pakeType +
                        " ./")
            ...
            cmd_line = " ./tcs/" + \
                str(pakeNo) + " -o ./opt/wrt-manifest-tizen-tests/" + \
                str(pakeNo) + "." + pakeType + " key.pem"
            if (pakeType == "xpk"):
                os.system(cmd_packing + cmd_line)
            else:
                os.chdir(os.getcwd() + "/tcs/" + str(pakeNo))
                os.system(
                    "zip -rq ../../opt/wrt-manifest-tizen-tests/" +
                    str(pakeNo) +
                    "." +
                    pakeType +
                    " ./")
...
def launcher_WebApp(
        pakeType, Manifest_Row, Device_ID, Device_Type, Manifest_Name):
    try:
        ...
        if (Device_Type == "sdb"):
            ...
            cmd_pushxpk = "sdb -s " + Device_ID + " push " + const.name + "-" + \
                const.version + "." + pakeType + ".zip " + const.device_path
            cmd_unzipxpk = "sdb -s " + Device_ID + " shell unzip -od " + const.device_path + \
                "  " + const.device_path + const.name + "-" + \
                const.version + "." + pakeType + ".zip "
            cmd_installapp = "sdb -s " + Device_ID + " shell \"su - " + Tizen_User + " -c 'export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/" + Userid + "/dbus/user_bus_socket;" + \
                const.device_path + "/opt/wrt-manifest-tizen-tests/appinstall.sh " + const.device_path + \
                "/opt/wrt-manifest-tizen-tests/" + \
                str(Manifest_Row) + "." + pakeType + "'\""
            ...
            cmd_chmod = "sdb -s " + Device_ID + " shell chmod 777 " + \
                const.device_path + \
                "/opt/wrt-manifest-tizen-tests/appinstall.sh"
            os.system(cmd_chmod)
            cmd_chmod = "sdb -s " + Device_ID + " shell chmod 777 " + \
                const.device_path + \
                "/opt/wrt-manifest-tizen-tests/applaunch.sh"
            os.system(cmd_chmod)
            cmd_chmod = "sdb -s " + Device_ID + " shell chmod 777 " + \
                const.device_path + \
                "/opt/wrt-manifest-tizen-tests/appuninstall.sh"
            os.system(cmd_chmod)
            cmd_chmod = "sdb -s " + Device_ID + " shell chmod 777 " + \
                const.device_path + "/opt/wrt-manifest-tizen-tests/checkdb.sh"
            os.system(cmd_chmod)
            cmd_chmod = "sdb -s " + Device_ID + " shell chmod 777 " + \
                const.device_path + \
                "/opt/wrt-manifest-tizen-tests/checkdb_new.sh"
            os.system(cmd_chmod)
            ...
            get_pkgid = get_cmdback1[
                (get_cmdback1.find("pkgid") + 6):(get_cmdback1.find("pkgid") + 38)]
            get_install_flag = get_cmdback1.find("val[ok]")

            # install ok and test =positive
            if ((get_install_flag > 0) & (Test_Flag == "positive")):
                print "Install---------> OK "
                fail_message = "install ok"
                auto_result = "Pass"
                # launcher app
                Pkgids = get_pkgid
                cmd_launchapp = "sdb -s " + Device_ID + " shell \"su - " + Tizen_User + " -c 'export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/" + \
                    Userid + "/dbus/user_bus_socket;" + const.device_path + \
                    "/opt/wrt-manifest-tizen-tests/applaunch.sh xwalk." + \
                    Pkgids + "'\""
                ...
                if ((get_cmdback[0].strip("\r\n")) == "Launch ok"):
                    ...
                    # uninstall app
                    cmd_uninstallapp = "sdb -s " + Device_ID + " shell \"su - " + Tizen_User + " -c 'export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/" + \
                        Userid + "/dbus/user_bus_socket;bash " + const.device_path + \
                        "/opt/wrt-manifest-tizen-tests/appuninstall.sh " + \
                        Pkgids + "'\""
                    get_cmdback = get_runback(
                        cmd_uninstallapp,
                        "uninstall",
                        "")
            ...
            # positive install ok but test fail
            elif ((Test_Flag == "positive")):
                auto_result = "FAIL"
            ...
            os.system(
                ...
                " shell rm -rf " +
                const.device_path +
                "/wrt-manifest-tizen-tests*")
        else:
            print "use ssh device-------------->"
            cmd_pushxpk = "scp " + const.name + "-" + const.version + "." + \
                pakeType + ".zip " + Device_ID + ":" + const.device_path
            cmd_unzipxpk = "ssh " + Device_ID + " 'unzip -od " + const.device_path + "  " + \
                const.device_path + const.name + "-" + \
                const.version + "." + pakeType + ".zip'"
            cmd_installapp = "ssh " + Device_ID + " \"su - " + Tizen_User + " -c 'export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/" + Userid + "/dbus/user_bus_socket;bash " + \
                const.device_path + "/opt/wrt-manifest-tizen-tests/appinstall.sh " + const.device_path + \
                "/opt/wrt-manifest-tizen-tests/" + \
                str(Manifest_Row) + "." + pakeType + "'\""
            ...
            if ((get_install_flag >= 0) & (Test_Flag == "positive")):
                ...
                # launcher app
                Pkgids = get_pkgid
                cmd_launchapp = "ssh " + Device_ID + " \"su - " + Tizen_User + " -c 'export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/" + \
                    Userid + "/dbus/user_bus_socket;bash " + const.device_path + \
                    "/opt/wrt-manifest-tizen-tests/applaunch.sh xwalk." + \
                    Pkgids + "'\""
                ...
                if ((get_cmdback[0].strip("\r\n")) == "Launch ok"):
                    ...
                    fail_message = "launch webapp ok"
                    # uninstall app
                    cmd_uninstallapp = "ssh " + Device_ID + "  \"su - " + Tizen_User + " -c 'export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/" + \
                        Userid + "/dbus/user_bus_socket;bash " + const.device_path + \
                        "/opt/wrt-manifest-tizen-tests/appuninstall.sh " + \
                        Pkgids + "'\""
                    get_cmdback = get_runback(
            ...
            # positive install ok but test fail
                ...
                get_cmdback = get_runback(cmd_uninstallapp, "uninstall", "")
                auto_result = "FAIL"
            # install ok and test =negative
            elif ((get_install_flag >= 0) & (Test_Flag == "negative")):
                auto_result = "FAIL"
