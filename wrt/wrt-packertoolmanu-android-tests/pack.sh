...
do
    if [ "${buildfolder:0:20}" == "webgl_webrtc_disable" ];then
        python make_apk.py --package=org.xwalk.$buildfolder --name=$buildfolder --manifest=$BUILD_DEST/opt/$name/$sourcepath/$buildfolder/manifest.json --mode=$mode --arch=$arch --xwalk-command-line='--disable-webgl --disable-webrtc'
        if [ $? -eq 0 ];then
            mv *apk $BUILD_ROOT/apkfolder/${buildfolder}_${arch}.apk
        fi
        continue
    fi
    echo "python make_apk.py --package=org.xwalk.$buildfolder --name=$buildfolder --app-root=$BUILD_DEST/opt/$name/$sourcepath/$buildfolder --app-local-path=index.html --mode=$mode --arch=$arch"
