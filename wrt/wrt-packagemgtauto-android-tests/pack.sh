...
do
        ...
        if [ "$buildfolder" == "packagemgt" ];then
            ...
            python make_apk.py --package=org.xwalk.$buildfolder --name=$buildfolder --app-url=http://www.baidu.com
            python make_apk.py --package=org.xwalk.apk2$buildfolder --name=apk2$buildfolder --app-url=http://www.baidu.com
            continue
        fi
done
}
...
cp -r $BUILD_ROOT/xwalk_app_template/*packagemgt*.apk $BUILD_DEST/opt/$name/source
cd $BUILD_DEST
