        ...
        while 1:
            ...
            for i in range(0,const.run_times):
                ...
                if (get_run_back=="testend"):
                    ...
                    if (os.path.isfile("opt/Crosswalk-Manifest-Check/manifest" + str(Manifest_Row) + "." + Pack_Type)):
                        log_Log(" webapp run fail and save webapp" + str(Manifest_Row) + "." + Pack_Type+ " to ./result" +"\n" )
                        shutil.move("opt/Crosswalk-Manifest-Check/manifest" + str(Manifest_Row) + "." + Pack_Type ,const.path_result)
...
def manifest_Packing(pakeNo,pakeType):
    try:
        ...
        do_Clear("./opt")
        os.system("rm -rf *.zip")
        os.makedirs("./opt/Crosswalk-Manifest-Check")
        shutil.copy(const.sh_path +"/appinstall.sh","./opt/Crosswalk-Manifest-Check/")
        shutil.copy(const.sh_path +"/applaunch.sh","./opt/Crosswalk-Manifest-Check/")
        shutil.copy(const.sh_path +"/appuninstall.sh","./opt/Crosswalk-Manifest-Check/")
        shutil.copy(const.sh_path +"/checkdb.sh","./opt/Crosswalk-Manifest-Check/")
        shutil.copy(const.sh_path +"/checkdb_new.sh","./opt/Crosswalk-Manifest-Check/")
        ...
        if (pakeNo =="all"):#all is not support now,please use default 
            for i in range (1,(Manifest_Row+1)):
                cmd_line=" ./tcs/manifest" + str(i) +" -o ./opt/Crosswalk-Manifest-Check/manifest" + str(i) + "."+ pakeType +" key.pem "
                if (pakeType=="xpk"):
                    os.system(cmd_packing + cmd_line)
                else:
                    os.chdir(os.getcwd() + "/tcs/manifest" + str(i))
                    os.system("zip -rq ../../opt/Crosswalk-Manifest-Check/manifest"+ str(i) +"." + pakeType+" ./")
                    os.chdir(const.path)
        else:
             cmd_line=" ./tcs/" + str(pakeNo) +" -o ./opt/Crosswalk-Manifest-Check/" + str(pakeNo)+"."+ pakeType +" key.pem"
             if (pakeType=="xpk"):
                    os.system(cmd_packing + cmd_line)
             else:
                    os.chdir(os.getcwd()+"/tcs/" + str(pakeNo))
                    os.system("zip -rq ../../opt/Crosswalk-Manifest-Check/" + str(pakeNo) +"." + pakeType+" ./")
                    os.chdir(const.path)
...
def launcher_WebApp(pakeType,Manifest_Row, tcs_manifest):        
    try:
        ...
        cmd_installapp="sdb -s " + Device_Ip +" shell \"su - app -c 'export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/5000/dbus/user_bus_socket;" + const.device_path + "/opt/Crosswalk-Manifest-Check/appinstall.sh "+ const.device_path +"/opt/Crosswalk-Manifest-Check/manifest" + str(Manifest_Row) +  "." + pakeType +"'\""
        print "------------push webapp----------->"
        ...
        cmd_chmod = "sdb -s " + Device_Ip +" shell chmod 777 "+const.device_path+"/opt/Crosswalk-Manifest-Check/appinstall.sh"
        os.system(cmd_chmod)
        cmd_chmod = "sdb -s " + Device_Ip +" shell chmod 777 "+const.device_path+"/opt/Crosswalk-Manifest-Check/applaunch.sh"
        os.system(cmd_chmod)        
        cmd_chmod = "sdb -s " + Device_Ip +" shell chmod 777 "+const.device_path+"/opt/Crosswalk-Manifest-Check/appuninstall.sh"
        os.system(cmd_chmod)
        cmd_chmod = "sdb -s " + Device_Ip +" shell chmod 777 "+const.device_path+"/opt/Crosswalk-Manifest-Check/checkdb.sh"
        os.system(cmd_chmod)
        cmd_chmod = "sdb -s " + Device_Ip +" shell chmod 777 "+const.device_path+"/opt/Crosswalk-Manifest-Check/checkdb_new.sh"
        os.system(cmd_chmod)
        ...
        #install app
        cmd_checkdb="sdb -s " + Device_Ip +" shell \"su - app -c 'export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/5000/dbus/user_bus_socket;" + const.device_path + "/opt/Crosswalk-Manifest-Check/checkdb.sh '\""
        ...
        cmd_checkdb_new = "sdb -s " + Device_Ip +" shell \"su - app -c 'export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/5000/dbus/user_bus_socket;"+const.device_path+"/opt/Crosswalk-Manifest-Check/checkdb_new.sh " + str(int(get_dbcount_after)-1) + "'\""
        ...
        if ((add_webapp==1) & (get_fromdb[0]=="GET") & (Test_Flag=="positive")): #install ok and test =positive
              ...
              #launcher app
              Pkgids = get_fromdb[1]
              cmd_launchapp="sdb -s " + Device_Ip +" shell \"su - app -c 'export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/5000/dbus/user_bus_socket;"+const.device_path+"/opt/Crosswalk-Manifest-Check/applaunch.sh " + Pkgids +"'\""
              ...
              if ((get_cmdback[0].strip("\r\n"))=="Launch ok"):
                  ...
                  #uninstall app
                  cmd_uninstallapp="sdb -s " + Device_Ip +" shell \"su - app -c 'export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/5000/dbus/user_bus_socket;"+const.device_path+"/opt/Crosswalk-Manifest-Check/appuninstall.sh " + Pkgids +"'\""
                  cmd_checkdb="sdb -s " + Device_Ip +" shell \"su - app -c 'export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/5000/dbus/user_bus_socket;"+const.device_path+"/opt/Crosswalk-Manifest-Check/checkdb.sh " + Pkgids +"'\""
                  ...
                  if (uninstall_webapp==0):
                      ...
                      return "testagain"
                      #shutil.move("opt/Crosswalk-Manifest-Check/manifest" + Manifest_Row + "." + pakeType ,const.path_result)
              else:
        ...
        os.system("sdb -s " + Device_Ip +" shell rm -rf "+const.device_path+"/opt/Crosswalk-Manifest-Check")
        os.system("sdb -s " + Device_Ip +" shell rm -rf "+const.device_path+"/Crosswalk-Manifest-Check*")
