...
<html>
  <body bgcolor="#d0d0d0">
    <h1 align="center">WebApp1</h1>
    <p>This page is the application's entry</p>
  </body>
</html>
EOF
sed -i 's/wrt-autodevice-tizen-tests/WebApp1/g' manifest.json
cd $xpkpacktooldir
python make_xpk.py /tmp/$name k.pem
if [ $? -ne 0 ];then
    echo "Create $name.xpk fail.... >>>>>>>>>>>>>>>>>>>>>>>>>"
    clean_workspace
    exit 1
fi
mv *.xpk $BUILD_DEST/opt/$name/autodevice/sources/WebApp1.xpk
#clean middle files
rm k.pem
cd $BUILD_DEST/
sed -i 's/WebApp1/WebApp2/g' manifest.json
sed -i 's/WebApp1/WebApp2/g' index.html
cd $xpkpacktooldir
python make_xpk.py /tmp/$name k.pem
if [ $? -ne 0 ];then
    echo "Create $name.xpk fail.... >>>>>>>>>>>>>>>>>>>>>>>>>"
...
mv *.xpk $BUILD_DEST/opt/$name/autodevice/sources/WebApp2.xpk
#clean middle files
rm k.pem
}
