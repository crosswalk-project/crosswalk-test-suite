...
do
    python make_apk.py --package=org.xwalk.$buildfolder --name=$buildfolder --app-root=$BUILD_DEST/opt/$name/$sourcepath/$buildfolder --app-local-path=$BUILD_DEST/opt/$name/$sourcepath/$buildfolder/index.html --extensions=$BUILD_DEST/opt/$name/$sourcepath/$buildfolder/contactextension --mode=$mode --arch=$arch
    if [ $? -eq 0 ];then
