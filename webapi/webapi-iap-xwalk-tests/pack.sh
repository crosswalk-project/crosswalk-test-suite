...
python make_apk.py --package=org.xwalk.$appname --permissions=iap --name=$appname --app-root=$BUILD_DEST --app-local-path=index.html --icon=$BUILD_DEST/icon.png --extensions=$BUILD_DEST/opt/webapi-iap-xwalk-tests/iap --mode=$mode --arch=$arch --manifest=$BUILD_DEST/opt/webapi-iap-xwalk-tests/manifest.json
if [ $? -ne 0 ];then
