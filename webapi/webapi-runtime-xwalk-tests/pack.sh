...
SubApkSrc1=$BUILD_ROOT/runtime/onLaunched
SubApkDir1=`basename $SubApkSrc1`
SubApkName1=`basename $SubApkSrc1 |sed 's/-/_/g'`_test

SubApkSrc2=$BUILD_ROOT/runtime/onSuspend
SubApkDir2=`basename $SubApkSrc2`
SubApkName2=`basename $SubApkSrc2 |sed 's/-/_/g'`_test

SubApkSrc3=$BUILD_ROOT/runtime/onInstalled
SubApkDir3=`basename $SubApkSrc3`
SubApkName3=`basename $SubApkSrc3 |sed 's/-/_/g'`_test
...
EOF
cp -a $BUILD_ROOT/icon.png     $BUILD_DEST/
cp -r $SubApkSrc1  $BUILD_DEST
cp -r $SubApkSrc2  $BUILD_DEST
...
python make_apk.py --package=org.xwalk.$SubApkName1 --name=$SubApkName1 --app-root=$BUILD_DEST/$SubApkDir1 --app-local-path=index.html --icon=$BUILD_DEST/$SubApkDir1/icon.png --mode=$mode
python make_apk.py --package=org.xwalk.$SubApkName2 --name=$SubApkName2 --app-root=$BUILD_DEST/$SubApkDir2 --app-local-path=index.html --icon=$BUILD_DEST/$SubApkDir2/icon.png --mode=$mode
python make_apk.py --package=org.xwalk.$SubApkName3 --name=$SubApkName3 --app-root=$BUILD_DEST/$SubApkDir3 --app-local-path=index.html --icon=$BUILD_DEST/$SubApkDir3/icon.png --mode=$mode
...
./cordova/build
if [ $? -ne 0 ]; then
    echo "Create $SubApkName1.apk fail.... >>>>>>>>>>>>>>>>>>>>>>>>>"
    clean_workspace
    exit 1
fi
...
./cordova/build
if [ $? -ne 0 ]; then
    echo "Create $SubApkName2.apk fail.... >>>>>>>>>>>>>>>>>>>>>>>>>"
    clean_workspace
    exit 1
fi
...
./cordova/build
if [ $? -ne 0 ]; then
    echo "Create $SubApkName3.apk fail.... >>>>>>>>>>>>>>>>>>>>>>>>>"
    clean_workspace
    exit 1
fi
}

SubXpkSrc1=$BUILD_ROOT/runtime/onLaunched
SubXpkDir1=`basename $SubXpkSrc1`
SubXpkName1="onLaunch_test"

SubXpkSrc2=$BUILD_ROOT/runtime/onSuspend
SubXpkDir2=`basename $SubXpkSrc2`
SubXpkName2="onSuspend_test"

SubXpkSrc3=$BUILD_ROOT/runtime/onInstalled
SubXpkDir3=`basename $SubXpkSrc3`
SubXpkName3="onInstalled_test"
...
cp -r $SubXpkSrc1  $BUILD_DEST
cp -r $BUILD_ROOT/manifest.json $BUILD_DEST/$SubXpkDir1
old1=`cat $BUILD_DEST/$SubXpkDir1/manifest.json |grep -w name |awk -F '"' '{print $4}'`
sed -i "s/$old1/$SubXpkName1/" $BUILD_DEST/$SubXpkDir1/manifest.json

cp -r $SubXpkSrc2  $BUILD_DEST
cp -r $BUILD_ROOT/manifest.json $BUILD_DEST/$SubXpkDir2
old2=`cat $BUILD_DEST/$SubXpkDir2/manifest.json |grep -w name |awk -F '"' '{print $4}'`
sed -i "s/$old2/$SubXpkName2/" $BUILD_DEST/$SubXpkDir2/manifest.json

cp -r $SubXpkSrc3  $BUILD_DEST
cp -r $BUILD_ROOT/manifest.json $BUILD_DEST/$SubXpkDir3
old3=`cat $BUILD_DEST/$SubXpkDir3/manifest.json |grep -w name |awk -F '"' '{print $4}'`
sed -i "s/$old3/$SubXpkName3/" $BUILD_DEST/$SubXpkDir3/manifest.json

cp $SRC_ROOT/../../tools/make_xpk.py $BUILD_ROOT/make_xpk.py
cd $BUILD_ROOT
python make_xpk.py /tmp/$name key
python make_xpk.py $BUILD_DEST/$SubXpkDir1 key2
python make_xpk.py $BUILD_DEST/$SubXpkDir2 key3
python make_xpk.py $BUILD_DEST/$SubXpkDir3 key4
if [ $? -ne 0 ];then
...
mv $BUILD_ROOT/cordova/$SubApkName1/bin/$SubApkName1-debug.apk $BUILD_DEST/opt/$name/$SubApkName1.apk
mv $BUILD_ROOT/cordova/$SubApkName2/bin/$SubApkName2-debug.apk $BUILD_DEST/opt/$name/$SubApkName2.apk
mv $BUILD_ROOT/cordova/$SubApkName3/bin/$SubApkName3-debug.apk $BUILD_DEST/opt/$name/$SubApkName3.apk
...
mv $BUILD_ROOT/$SubXpkDir1.xpk $BUILD_DEST/opt/$name/
mv $BUILD_ROOT/$SubXpkDir2.xpk $BUILD_DEST/opt/$name/
mv $BUILD_ROOT/$SubXpkDir3.xpk $BUILD_DEST/opt/$name/
