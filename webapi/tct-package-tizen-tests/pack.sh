...
SubApkSrc=$BUILD_ROOT/package/support/TCTPackageInfoTest
SubApkDir=`basename $SubApkSrc`
SubApkName=$SubApkDir
...
python make_apk.py --package=org.xwalk.$SubApkName --name=$SubApkName --app-root=$BUILD_DEST/$SubApkDir --app-local-path=index.html --icon=$BUILD_DEST/$SubApkDir/icon.png --mode=$mode --arch=$arch
...
SubXpkSrc=$BUILD_ROOT/package/support/TCTPackageInfoTest
SubXpkDir=`basename $SubXpkSrc`
SubXpkName="TCTPackageInfoTest"
...
cd $BUILD_ROOT
python make_xpk.py $BUILD_DEST/$SubXpkDir key1
...
mv $BUILD_ROOT/$SubXpkDir.xpk $BUILD_DEST/opt/$name/
