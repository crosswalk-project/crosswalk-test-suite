...
def main(request, response):
    try:
        ...
        allowed_links = [
            ...
            "http://example.not",
            "foobar:someone@example.org"]
        if location not in allowed_links:
            local_parsed = urlparse(location)
            request_parsed = urlparse(request.url)
            if local_parsed.hostname is not None and local_parsed.hostname != request_parsed.hostname and local_parsed.hostname != (
                    "www2." + request_parsed.hostname):
