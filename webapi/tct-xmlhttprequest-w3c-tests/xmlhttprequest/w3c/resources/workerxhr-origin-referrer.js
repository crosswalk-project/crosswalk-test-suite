...
xhr.open('GET', 'inspect-headers.cgi?filter_name=referer', true)
xhr.send()
...
xhr2.open('GET', location.protocol + '//www2.'+location.hostname+(location.pathname.replace(/[^/]*$/, ''))+'inspect-headers.cgi?filter_name=origin&cors', true)
xhr2.send()

// If "origin" / base URL is the origin of this JS file, we can load files 
// from the server it originates from.. and requri.cgi will be able to tell us
...
xhr3.open('GET', 'requri.cgi?full', true)
xhr3.send()
