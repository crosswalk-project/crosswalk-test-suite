...
def main(request, response):
    try:
        ...
        def file_get_contents(path):
          file_object = open(os.path.split(os.path.realpath(__file__))[0] + os.path.sep + path)
          try:
        ...
        operator = {"audio": audio, "css": css,"font": font,"iframe": iframe,"image": image,"script": script}

        if operator.has_key(get_type):
