    ...
    for item in glob.glob("%s/*" % SCRIPT_DIR):
        if item.endswith(".wgt"):
            ...
            if not doRemoteCopy(item, "%s/%s" % (PKG_SRC_DIR, item_name)):
                ...
                action_status = False
    for item in glob.glob("%s/resourcetiming/w3c/submission/resources/cgi/*" % SCRIPT_DIR):
        item_name = os.path.basename(item)
        if not doRemoteCopy(item, "%s/resourcetiming/w3c/submission/resources/%s" % (PKG_SRC_DIR, item_name)):
        #if not doRemoteCopy(item, PKG_SRC_DIR):
            action_status = False
