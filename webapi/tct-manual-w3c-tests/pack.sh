...
<!doctype html>
<head>
    <meta http-equiv="Refresh" content="1; url=http://127.0.0.1/opt/$name/webrunner/index.html?testsuite=$RESOURCE_DIR/tct/opt/$name/tests.xml">
</head>
EOF
...
python make_apk.py --package=org.xwalk.$appname --name=$appname --app-root=$BUILD_DEST --app-local-path=index.html --icon=$BUILD_DEST/icon.png --arch=$arch
if [ $? -ne 0 ];then
