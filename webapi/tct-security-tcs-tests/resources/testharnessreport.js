...
add_result_callback(function(tob) {
	try {
		if(tob.status === tob.NOTRUN) return;
		var s_status = {0: 'PASS', 1: 'FAIL', 2: 'TIMEOUT', 3: 'NOT RUN'}[tob.status];
		console.log(tob.name + ' RESULT ' + tob.status + ' ' + s_status + ' ' + tob.message || '');

                //console.log(window.parent.statusFrame.src);
                //console.log(window.parent.statusFrame.contentDocument.URL);
                //console.log(window.parent.statusFrame.contentDocument.getElementsByTagName('input'));

                buttons = [
                    window.parent.statusFrame.contentDocument.getElementById('passradio'),
                    window.parent.statusFrame.contentDocument.getElementById('failradio'),
                    window.parent.statusFrame.contentDocument.getElementById('blockradio')
                ]
                if(buttons[0]) {
                    switch(tob.status) {
                        case 0: idx = 0; break;
                    ...
                    buttons[idx].tag = 1;
                    buttons[(idx + 1) % 3].checked = false;
                    buttons[(idx + 1) % 3].tag = 0;
                    buttons[(idx + 2) % 3].checked = false;
                    buttons[(idx + 2) % 3].tag = 0;
                }
	} catch(error) {
		console.log(tob.name + ' result_callback() error: ' + error);
	}
});
